...
namespace _1188.SCMS.Web
{
    ...
    public class UserRegistrationService : DomainService
    {
        ...
        /// </summary>
        public const string DefaultRole = "Team Member";
        ...
        [Invoke]
        public string[] GetAssignedRoles( string userName )
        {
        ...
        [Invoke]
        public string[] GetUnassignedRoles( string userName )
        {
            var userRoles = Roles.GetRolesForUser( userName );
            var allRoles = Roles.GetAllRoles();
            return RemoveDuplicates( userRoles, allRoles );
        }
        ...
        {
            var userm = Membership.GetUser( userName );
            ...
            return userm.ChangePassword( oldPassword, newPassword );
        }
        ...
        [Invoke]
        public void SyncronyzeRoles( string[] assignedRoles, string[] unassignedRoles, string user )
        {
            var userRolesDict = new Dictionary<string, bool>();
            var userRoles = Roles.GetRolesForUser( user );
            foreach ( var r in userRoles )
                userRolesDict[ r ] = true;

            var unassignedRolesDict = new Dictionary<string, bool>();
            foreach ( var r in unassignedRoles )
                unassignedRolesDict[ r ] = true;

            var assignedRolesDict = new Dictionary<string, bool>();
            foreach ( var r in assignedRoles )
                assignedRolesDict[ r ] = true;

            var rolesToDeleteDict = userRolesDict.Where( r => unassignedRolesDict.ContainsKey( r.Key ) ).ToDictionary( r => r.Key, r => r.Value );
            foreach ( var r in rolesToDeleteDict )
                Roles.RemoveUserFromRole( user, r.Key );

            var rolesToAddDict = assignedRolesDict.Where( r => !userRolesDict.ContainsKey( r.Key ) ).ToDictionary( r => r.Key, r => r.Value );
            foreach ( var r in rolesToAddDict )
                Roles.AddUserToRole( user, r.Key );
            
        }

        static string[] RemoveDuplicates( string[] arr1, string[] arr2 )
        {
            ...
            var finalDict = dict.Where( b => b.Value == true ).ToDictionary( b => b.Key, b => b.Value );

            var result = new string[ finalDict.Keys.Count ];
            finalDict.Keys.CopyTo( result, 0 );
            return result;
        }
    }
    ...
    //    public UserContainer(CreateUserStatus status, object userId)
    //    {
    //        this.Status = status;
    //        this.UserId = userId;
    //    }
